...
class Timesheets_api extends App_Controller
{
	protected $encryption_key = 'My@Very$trong&PrivateKey123';
    
    public function __construct()
    {
        parent::__construct();
        $this->load->model('attendancecore_model');
	...
	function secure_encrypt($data) {
        $encryption_key = hash('sha256', $this->encryption_key); // Change this key
	...
	function secure_decrypt($data) {
        ...
        $encryption_key = hash('sha256', $this->encryption_key); // Same key as used above
	...
	public function authenticate() {
		...
		try {
			$user = $this->attendancecore_model->authenticate($email, $password);
			...
			$checkin_result = $this->check_in_ts(1, $user->staffid);
			
			// return $this->output
			// 	->set_status_header(200)
			// 	->set_output(json_encode([
			// 		'status' => true,
			// 		'staff_id' => $this->secure_encrypt($user->staffid)
			// 	]));
			
            $staff_id = $user->staffid;
			$profile_filename = $user->profile_image ?? null;
			
            $fallback_image = base_url('assets/images/user-placeholder.jpg');

			$small_profile_path = $profile_filename
				? base_url('uploads/staff_profile_images/' . $staff_id . '/small_' . $profile_filename)
				: $fallback_image;

			$thumb_profile_path = $profile_filename
				? base_url('uploads/staff_profile_images/' . $staff_id . '/thumb_' . $profile_filename)
				: $fallback_image;
			...
			->set_output(json_encode([
				...
				'staff_id' => $user->staffid, // or $this->secure_encrypt($user->staffid)
				'firstname' => $user->firstname,
				'lastname' => $user->lastname,
				'job_position' => $user->position_name ?? null,
                'profile_small' => $small_profile_path,
    			'profile_thumb' => $thumb_profile_path,
	...
	public function check_in_ts($type_check, $staff_id, $data_override = null) {
		$data = $this->input->post();

		if (empty($data) && is_array($data_override)) {
			$data = $data_override;
		}
		...
		$re = $this->check_in($data);

		$response = [];

		if (is_numeric($re)) {
			if ($re == 2) {
				$response = ['status' => 'warning', 'message' => _l('your_current_location_is_not_allowed_to_take_attendance')];
			} elseif ($re == 3) {
				$response = ['status' => 'warning', 'message' => _l('location_information_is_unknown')];
			} elseif ($re == 4) {
				$response = ['status' => 'warning', 'message' => _l('route_point_is_unknown')];
			} elseif ($re == 5) {
				$response = ['status' => 'danger', 'message' => _l('ts_access_denie')];
			} elseif ($re == 6) {
				$response = ['status' => 'danger', 'message' => _l('ts_cannot_get_client_ip_address')];
			}
		} else {
			if ($re === true) {
				if ($type == 1) {
					$response = ['status' => 'success', 'message' => _l('check_in_successfull'), 'datetime' => date('Y-m-d H:i:s')];
				} else {
					$response = ['status' => 'success', 'message' => _l('check_out_successfull')];
				}
			} else {
				if ($type == 1) {
					$response = ['status' => 'warning', 'message' => _l('check_in_not_successfull')];
				} else {
					$response = ['status' => 'warning', 'message' => _l('check_out_not_successfull')];
				}
			}
		}
    ...
    // public function check_in_ts($type_check, $staff_id) {
	// 	// echo 1; 
    //     if ($this->input->post()) {
	// 		// echo 2; exit;
    //         $data = $this->input->post();
	...
	public function check_in($data)
	{
		...
		unset($data['date']);
		$date = date('Y-m-d'); // Just the date part
		$datetime = date('Y-m-d H:i:s'); // Full timestamp
