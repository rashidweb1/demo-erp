<script type="text/javascript">
			...
			if (response.shipment_log_id) {
				if(typeof(expenseDropzone) !== 'undefined'){
					if (expenseDropzone.getQueuedFiles().length > 0) {
						expenseDropzone.options.url = admin_url + 'fixed_equipment/add_shipment_attachment/' + response.shipment_log_id+'/'+response. cart_id;
	...
	function delete_product_attachment(wrapper, attachment_id, rel_type) {
		...
		if (confirm_delete()) {
			$.get(admin_url + 'fixed_equipment/delete_product_attachment/' +attachment_id+'/'+rel_type, function (response) {
