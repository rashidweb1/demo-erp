...
var croppedCtx;
  ...
  <?php if(isset($send_mail_approve)){ 
    ...
    $.get(admin_url+'fixed_equipment/send_mail', data_send_mail).done(function(response){
    ...
    $('select[name="customer_name"]').on('change',function(){
      ...
      var data_select = {};
        data_select.userid = $('select[name="customer_name"]').val();
          ...
          $.post(admin_url + 'fixed_equipment/get_primary_contact',data_select).done(function(response){
             response = JSON.parse(response);
  ...
  function change_request_approval_status(id, status, sign_code){
    ...
    var data = {};
    data.rel_id = id;
    data.rel_type = 'inventory_delivery';
    data.note = $('textarea[name="reason"]').val();
    data.approve = status;
    $.post(admin_url + 'fixed_equipment/approve_request/' + id, data).done(function(response){
      response = JSON.parse(response); 
  ...
  $("body").append('<div class="dt-loader"></div>');
    $.post(admin_url + 'fixed_equipment/send_request_approve', data).done(function(response){
        response = JSON.parse(response);
...
function accept_action() {
 ...
 function get_goods_delivery_ajax(id, invoice_id) {
  "use strict"; 
    var data={};
    data.id =  id;
     $.post(admin_url + 'fixed_equipment/get_delivery_ajax', data).done(function(response){
          response = JSON.parse(response);
...
function delete_wh_activitylog(wrapper, id) {
      "use strict"; 
    if (confirm_delete()) {
        requestGetJSON('fixed_equipment/delete_activitylog/' + id).done(function(response) {
