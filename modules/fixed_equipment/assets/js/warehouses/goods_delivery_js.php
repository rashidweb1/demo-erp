...
"use strict";  
  ...
  appValidateForm($('#add_goods_delivery'), {
   ...
   // Maybe items ajax search
    init_ajax_search('items','#item_select.ajax-search',undefined,admin_url+'fixed_equipment/wh_commodity_code_search/rate');
...
$("body").on('change', 'select[name="warehouse_id"]', function() {
  "use strict"; 
  if(data_reponse != null){
    var val = $(this).val();
    var selected_quantities = '';
    $.each(data_reponse.warehouse, function (i, row) {
      if(val != '' && val == row.id){
        selected_quantities = row.quantity;
        return false;
      }
    });
    $('.main input[name="available_quantity"]').val(selected_quantities);
    var quantity = 1;
    if(selected_quantities == ''){
      quantity = '';
    }
    $('.main input[name="quantities"]').val(quantity).attr('max', selected_quantities);
  }
});
...
function check_quantity_inventory(commodity_id, quantity, warehouse_id, switch_barcode_scanners = false) {
  ...
  if(commodity_id != '' && warehouse_id != '' ){
    $.post(admin_url + 'fixed_equipment/check_quantity_inventory', data).done(function(response){
...
var data_reponse = null;
// Add item to preview
function wh_add_item_to_preview(id) {
  "use strict";
  data_reponse = null;
  requestGetJSON('fixed_equipment/get_item_by_id_inventory_delivery/' + id +'/'+true).done(function (response) {
    clear_item_preview_values();
    $('.main input[name="commodity_code"]').val(id);
    $('.main textarea[name="commodity_name"]').val(response.item_name);
    $('.main input[name="unit_price"]').val(response.unit_price);
    $('.main input[name="available_quantity"]').val(response.selected_quantities);
    $('.main input[name="quantities"]').val(1).attr('max', response.selected_quantities);
    data_reponse = response;
    // var warehouse_option = '<option value=""></option>';
    // $.each(response.warehouse, function (i, row) {
    //   warehouse_option += '<option value="'+row.id+'">'+row.name+'</option>';
    // });

    $('.main select[name="warehouse_id"]').val(response.selected_warehouse_id).change();
    $('.selectpicker').selectpicker('refresh');

    if(response.item_type == 'asset'){
      $('.main input[name="quantities"]').attr('readonly', 'readonly');
    }
    else{
...
function wh_get_item_row_template(data)  {
  "use strict";
  ...
  var d = $.post(admin_url + 'fixed_equipment/get_good_delivery_row_template', data);
  jQuery.ajaxSetup({
    async: true
  });
  return d;
...
function invoice_change(){
  ...
  $.post(admin_url + 'fixed_equipment/copy_invoices/'+invoice_id).done(function(response){
    response = JSON.parse(response);
  ...
  function pr_order_change(){
    ...
    var pr_order_id = $('select[name="pr_order_id"]').val();
      ...
      $.post(admin_url + 'fixed_equipment/goods_delivery_copy_pur_order/'+pr_order_id).done(function(response){
        response = JSON.parse(response);
      ...
      if(pr_order_id != ''){
        $.post(admin_url + 'fixed_equipment/copy_pur_vender/'+pr_order_id).done(function(response){
      ...
      if (pressed == false) {
        setTimeout(function() {
          if (chars.length >= 8) {
            ...
            requestGetJSON('fixed_equipment/wh_get_item_by_barcode/' + barcode).done(function (response) {
  ...
  function wh_add_item_to_table(data, itemid) {
    ...
    data.name = 'newitems[' + item_key + ']';
    $("body").append('<div class="dt-loader"></div>');
    wh_get_item_row_template(data).done(function(output){
      table_row += output;
      $('.invoice-item table.invoice-items-table.items tbody').append(table_row);
  ...
  function wh_change_serial_number(name_commodity_code, name_warehouse_id, name_serial_number, name_commodity_name) {
    ...
    // get serial number
    $.post(admin_url + 'fixed_equipment/get_serial_number_for_change_modal', data_post).done(function(response){
      response = JSON.parse(response);
      if(response.status == true || response.status == 'true'){
  ...
  function open_change_serial_number_modal(table_serial_number, name_commodity_name, name_serial_number) {
    ...
    $("#change_serial_modal_wrapper").load("<?php echo admin_url('fixed_equipment/fixed_equipment/load_change_serial_number_modal'); ?>", {
      table_serial_number: table_serial_number,
