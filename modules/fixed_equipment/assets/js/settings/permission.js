(function(){
	"use strict";
	initDataTable('.table-hr-profile-permission', admin_url + 'fixed_equipment/permission_table');
		...
		$(document).on("change",'select[name="staff_id"]',function() {
			...
			if(staff_id){
				...
				requestGetJSON('fixed_equipment/staff_id_changed/' + staff_id).done(function(response) {
       ...
       function permissions_update(staff_id, role_id, add_new) {
       	"use strict";
       	$("#modal_wrapper").load(admin_url+"fixed_equipment/permission_modal", {
       		slug: 'update',
       		staff_id: staff_id,
       		role_id: role_id,
       		add_new: add_new
