<script type="text/javascript">
	function fixed_equipment_order_list_status_mark_as(status, task_id, type) {
		...
		"use strict";  
		if (confirm_delete()) {
			$.get(site_url + 'fixed_equipment/fixed_equipment_client/delete_issue_pdf_file/' +attachment_id, function (response) {
	...
	function view_file(id, rel_id) {   
		...
		$('#pdf_file_data').empty();
		$("#pdf_file_data").load(site_url + 'fixed_equipment/fixed_equipment_client/view_pdf_file/' + id + '/' + rel_id , function(response, status, xhr) {
			if (status == "error") {
