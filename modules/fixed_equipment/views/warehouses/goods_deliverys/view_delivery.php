 ...
 <div class="horizontal-scrollable-tabs preview-tabs-top">
  ...
  <div class="horizontal-tabs">
   ...
   <li role="presentation">
     <a href="#tab_tasks" onclick="init_rel_tasks_table(<?php echo fe_htmldecode($goods_delivery->id); ?>,'stock_export'); return false;" aria-controls="tab_tasks" role="tab" data-toggle="tab">
       <?php echo _l('tasks'); ?>
     </a>
   </li>
    ...
    <div class="row">
      ...
      <div class="col-md-8">
       <div class="pull-right _buttons">
        <?php if(has_permission('fixed_equipment_inventory','','view') || is_admin()){ ?>
         <!--  <a href="<?php //echo admin_url('fixed_equipment/edit_delivery/'.$goods_delivery->id); ?>" class="btn btn-default btn-with-tooltip" data-toggle="tooltip" title="<?php //echo _l('view'); ?>" data-placement="bottom"><i class="fa fa-eye"></i></a> -->
       <?php } ?>
    ...
    <table class="table border table-striped table-margintop ">
      <tbody>
        ...
        <tr class="project-overview">
          ...
          <td><?php echo fe_htmldecode($customer_name) ; ?></td>
        </tr>
        <tr class="project-overview">
          <td class="bold"><?php echo _l('to'); ?></td>
          <td><?php echo fe_htmldecode($goods_delivery->to_) ; ?></td>
        </tr>
        <tr class="project-overview">
          <td class="bold"><?php echo _l('address'); ?></td>
          <td><?php echo fe_htmldecode($goods_delivery->address) ; ?></td>
        </tr>
        <tr class="project-overview">
          <td class="bold"><?php echo _l('note_'); ?></td>
          <td><?php echo fe_htmldecode($goods_delivery->description) ; ?></td>
        </tr>
   ...
   <div class="col-md-12">
    <div class="table-responsive">
     <table class="table items items-preview estimate-items-preview" data-type="estimate">
      <thead>
       <tr>
        <th align="center">#</th>
        <th><?php echo _l('fe_item') ?></th>
        <th><?php echo _l('fe_warehouse') ?></th>
        <th><?php echo _l('fe_available_quantity') ?></th>
        <th class="text-center"><?php echo _l('fe_quantity') ?></th>
        <th align="right"><?php echo _l('rate') ?></th>
        <th align="right"><?php echo _l('subtotal') ?></th>
        <th align="right"><?php echo _l('subtotal_after_tax') ?></th>
        <th align="right"><?php echo _l('discount').'(%)' ?></th>
        <th align="right"><?php echo _l('discount(money)') ?></th>
        <th align="right"><?php echo _l('total_money') ?></th>
      </tr>
    </thead>
    <tbody class="ui-sortable">
      ...
      foreach ($goods_delivery_detail as $delivery => $delivery_value) {
       $commodity_name = fe_item_name($delivery_value['commodity_code'], true);
       $item_subtotal = (float)$delivery_value['quantities'] * (float)$delivery_value['unit_price'];
       $subtotal += $item_subtotal;
       $warehouse_name = fe_get_warehouse_name($delivery_value['warehouse_id']);
       ?>

       <tr>
        <td class=""><?php echo fe_htmldecode($delivery + 1); ?></td>
        <td class=""><?php echo fe_htmldecode($commodity_name); ?></td>
        <td class=""><?php echo fe_htmldecode($warehouse_name); ?></td>
        <td class="text-right"><?php echo fe_htmldecode($delivery_value['available_quantity']); ?></td>
        <td class="text-right"><?php echo fe_htmldecode($delivery_value['quantities']); ?></td>
        <td class="text-right"><?php echo app_format_money((float)$delivery_value['unit_price'],''); ?></td>
        <td class="text-right"><?php echo app_format_money((float)$item_subtotal,''); ?></td>
        <td class="text-right"><?php echo app_format_money((float)$delivery_value['total_money'],''); ?></td>
        <td class="text-right"><?php echo app_format_money((float)$delivery_value['discount'],''); ?></td>
        <td class="text-right"><?php echo app_format_money((float)$delivery_value['discount_money'],''); ?></td>
        <td class="text-right"><?php echo app_format_money((float)$delivery_value['total_after_discount'],''); ?></td>
      </tr>
    <?php  } ?>
  </tbody>
</table>
     ...
     <div class="row">
       <div class="col-md-12 project-overview-expenses-finance">
           ...
           <div class="col-md-3 text-center">
            ...
            <?php if($value['approve'] == 1){ ?>
              <?php if (file_exists(WAREHOUSE_STOCK_EXPORT_MODULE_UPLOAD_FOLDER . $goods_delivery->id . '/signature_'.$value['id'].'.png') ){ ?>
                <img src="<?php echo site_url('modules/fixed_equipment/uploads/stock_export/'.$goods_delivery->id.'/signature_'.$value['id'].'.png'); ?>" class="img-width-height">
              <?php }else{ ?>
                <img src="<?php echo site_url('modules/fixed_equipment/uploads/image_not_available.jpg'); ?>" class="img-width-height">
              <?php } ?>
            <?php }
            ?> 
        ...
        <?php }else{ ?>
          ...
          <?php if($value['approve'] == 1){ 
            ?>
            <img src="<?php echo site_url('modules/fixed_equipment/uploads/approve/approved.png') ; ?>" class="img-width-height">
          <?php }elseif($value['approve'] == -1){ ?>
            <img src="<?php echo site_url('modules/fixed_equipment/uploads/approve/rejected.png') ; ?>" class="img-width-height">
          <?php }
          ?>  
  ...
  if($goods_delivery->approval != 1 && ($check_approve_status == false ))
    ...
    { ?>
      <?php if($check_appr && $check_appr != false){ ?>
        <a data-toggle="tooltip"  class="btn btn-success lead-top-btn lead-view send_request_approve_class" data-placement="top" href="#" onclick="send_request_approve(<?php echo fe_htmldecode($goods_delivery->id); ?>); return false;"><?php echo _l('send_request_approve'); ?></a>
      <?php } ?>
         ...
         <ul class="dropdown-menu dropdown-menu-right menu-width-height" >
          ...
          <li>
            <div class="row text-right col-md-12">
              <a href="#" data-loading-text="<?php echo _l('wait_text'); ?>" onclick="approve_request(<?php echo fe_htmldecode($goods_delivery->id); ?>); return false;" class="btn btn-success button-margin" ><?php echo _l('approve'); ?></a>
              <a href="#" data-loading-text="<?php echo _l('wait_text'); ?>" onclick="deny_request(<?php echo fe_htmldecode($goods_delivery->id); ?>); return false;" class="btn btn-warning"><?php echo _l('deny'); ?></a></div>
            </li>
          </ul>
        </div>
      <?php }
      ...
      <div class="col-md-12">
        <div class="table-responsive">
          <table class="table items items-preview estimate-items-preview" data-type="estimate">
            <thead>
              <tr>
                <th  colspan="1"><?php echo _l('fe_packing_list_number') ?></th>
                <th  colspan="1"><?php echo _l('customer_name') ?></th>
                <th align="right" colspan="1"><?php echo _l('fe_dimension') ?></th>
                <th align="right" colspan="1"><?php echo _l('volume_m3_label') ?></th>
                <th align="right" colspan="1"><?php echo _l('total_amount') ?></th>
                <th align="right" colspan="1"><?php echo _l('fe_discount_total') ?></th>
                <th align="right" colspan="1"><?php echo _l('fe_total_after_discount') ?></th>
                <th align="right" colspan="1"><?php echo _l('fe_date_created') ?></th>
            ...
            <tbody class="ui-sortable">
              ...
              foreach ($packing_lists as $key => $packing_list) {
                ...
                <tr>
                  <td ><a href="<?php echo admin_url('fixed_equipment/view_packing_list/' . $packing_list['id'] ) ?>" ><?php echo fe_htmldecode($packing_list['packing_list_number']) ?></a></td>
                  <td ><?php echo get_company_name($packing_list['clientid']) ?></td>
                  <td class="text-right"><?php echo fe_htmldecode($packing_list['width'].' x '.$packing_list['height'].' x '.$packing_list['lenght']) ?></td>
   ...
   <?php foreach($activity_log as $log){ ?>
     <div class="feed-item">
      <div class="date">
        ...
        <?php if($log['staffid'] == get_staff_user_id() || is_admin() || has_permission('fixed_equipment_inventory','','delete')){ ?>
          <a href="#" class="pull-right text-danger" onclick="delete_wh_activitylog(this,<?php echo fe_htmldecode($log['id']); ?>);return false;"><i class="fa fa fa-times"></i></a>
        <?php } ?>
      </div>
      <div class="text">
       ...
       if(!empty($log['additional_data'])){
        ...
        echo fe_htmldecode($log['full_name']) . ' - ';
...
<div class="modal fade" id="add_action" tabindex="-1" role="dialog">
 <div class="modal-dialog">
  ...
  <div class="modal-footer">
    ...
    <button onclick="sign_request(<?php echo fe_htmldecode($goods_delivery->id); ?>);" autocomplete="off" class="btn btn-success sign_request_class"><?php echo _l('e_signature_sign'); ?></button>
  </div>
</div>
</div>
</div>
</div>
<?php require 'modules/fixed_equipment/assets/js/warehouses/view_delivery_js.php';?>
</body>
</html>